...
- An MCP header whose value references an environment variable now reaches Codex in a form Codex resolves at server start: `Authorization: "Bearer ${VAR}"` is written as `bearer_token_env_var`, and a value that is exactly `${VAR}` as `env_http_headers`. Codex documents `http_headers` as static values, so APM previously wrote the unexpanded placeholder there and the server received it as literal text, failing to authenticate for Codex while the same dependency worked for Claude Code. A value mixing literal text with a placeholder has no Codex equivalent and is now skipped with a warning instead of written as a literal. `bearer_token_env_var` and `env_http_headers` join `http_headers` as reserved passthrough keys, so an `extra:` block cannot inject an Authorization header past `headers` modeling. -- by @edenfunf (#2984)
