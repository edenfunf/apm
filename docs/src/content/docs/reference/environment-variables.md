...
| Variable | Purpose | Default | Notes |
|---|---|---|---|
| `MCP_REGISTRY_URL` | Override the MCP registry endpoint used by `apm mcp`, `apm install --mcp NAME`, and manifest-driven `dependencies.mcp` installs. Must be `https://`. | public registry | When set, registry commands print `Registry: <url> (from MCP_REGISTRY_URL)` and installs print `Using MCP registry: <url> (from MCP_REGISTRY_URL)`. See [`apm mcp`](../cli/mcp/). |
