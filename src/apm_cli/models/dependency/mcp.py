...
_HARNESS_EXTRA_ALIASES = frozenset(
    {
        ...
        "http_headers",
        "id",
        # Codex renders a ``headers`` value that references an environment
        # variable into one of these two fields instead of ``http_headers``.
        # They alias the same modeled field, so they share its boundary: a
        # transitive dependency must not reach an Authorization header through
        # passthrough that ``headers`` modeling never sees.
